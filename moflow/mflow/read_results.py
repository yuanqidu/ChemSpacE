    ...
    for d in data_names:
        for s in mani_steps:
            ...
            for line in f:
                ...
                elif line.startswith('total soft global success'):
                    soft_success.append(float(line.split(" ")[-1]))
                elif line.startswith('total soft local success'):
                    soft_success_local.append(float(line.split(" ")[-1]))
            ...
            soft = os.path.join(base_dir, f'{d}_{m}_{s}_soft_global.txt')
            soft_local = os.path.join(base_dir, f'{d}_{m}_{s}_soft_local.txt')
            ...
            for prop in prop_name:
                ...
                for line in f:
                    if prop in line and prop == line.split(" ")[0]:
                        props_result_soft_local.append(float(line.split(" ")[-1]))
            
            print (f'{d}_{m}_{s} result report')
            table_full = ""
            strict = np.mean(strict_success)
            soft_local = np.mean(soft_success_local)
            ...
            table_ind_strict = f"soft local/soft global {soft_local:.2f} / {soft:.2f} & "
            for i, prop in enumerate(props_result_soft):
                table_ind_strict += f" {props_result_soft_local[i]:.2f} / {props_result_soft[i]:.2f} &"
            print (table_ind_strict)

            table_ind_strict = f"strict {strict:.2f}& "
            for i, prop in enumerate(props_result_strict):
                table_ind_strict += f" {props_result_strict[i]:.2f}&"
            print (table_ind_strict)
