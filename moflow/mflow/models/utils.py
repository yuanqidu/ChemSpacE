...
def construct_mol(x, A, atomic_num_list):
    ...
    for start, end in zip(*np.nonzero(adj)):
        if start > end:
            ...
            # print('Enter check valency in construct mol')
            flag, atomid_valence = check_valency(mol)
            # print('Exit check valency in construct mol')
            if flag:
                continue
            else:
...
def correct_mol(x):
    ...
    counter = 0
    while True:
        print('Enter check valency')
        flag, atomid_valence = check_valency(mol)
        print('Exit check valency')
        if flag:
            break
        else:
...
def check_validity(adj, x, atomic_num_list, gpu=-1, return_unique=True,
    ...
    if correct_validity:
        ...
        valid = []
        for x_elem, adj_elem in zip(x, adj):
            print('Enter construct mol')
            mol = construct_mol(x_elem, adj_elem, atomic_num_list)
            print('Exit construct mol')
            # Chem.Kekulize(mol, clearAromaticFlags=True)
            print('Enter correct mol')
            cmol = correct_mol(mol)
            print('Exit correct mol')
            print('Enter valid mol can with seg')
            ...
            print('Exit valid mol can with seg')
