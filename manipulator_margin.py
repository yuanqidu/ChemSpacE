...
def train_boundary(latent_codes,
  ...
  idx_min = scores<=mean_score-std_score
  print('Idx_min:', idx_min.size)
  idx_max = scores>=mean_score+std_score
  print('Idx_max:', idx_max.size)
  idx = [idx_min[i][0] or idx_max[i][0] for i in range(len(idx_min))]
  idx = np.array(idx)
  print ('Sum of Min and Max groups: ', idx_min.sum(), idx_max.sum())
  print ('Mean of Min and Max groups: ', idx_min.mean(), idx_max.mean())
  ...
  if (not isinstance(scores, np.ndarray) or not len(scores.shape) == 2 
  ...
  print (remaining_scores[remaining_scores<=mean_score-std_score].size)
  print (remaining_scores[remaining_scores>=mean_score+std_score].size)
  # decision_value = (scores[0] + scores[-1]) / 2
  print (remaining_scores.mean(), remaining_scores.std(), mean_score, std_score)
  remaining_label = np.ones(remaining_num, dtype=np.int)
  remaining_label[remaining_scores.ravel() >= mean_score+std_score] = 1
  remaining_label[remaining_scores.ravel() <= mean_score-std_score] = 0
  ...
  correct_num = np.sum(train_prediction == train_label)
  ...
  return a / np.linalg.norm(a), train_num, time_end-time_begin
